...
std::string GDrawSomething(GCanvas* canvas, GISize dim) {
    GBitmap bm;
    bm.readFromFile("apps/tar_heels.png");
    float w = bm.width();
    float h = bm.height();
    ...
    GBitmap bm2;
    bm2.readFromFile("apps/tar_heels.png");
    float w2 = bm2.width();
    float h2 = bm2.height();
    ...
    auto sh2 = GCreateBitmapShader(bm2, GMatrix());
    GPaint paint2(sh2.get());

    canvas->translate(120, 120);
    canvas->rotate(M_PI);
    canvas->translate(-120, -120);
    canvas->drawMesh(pts2, nullptr, tex2, 1, indices2, paint2);  

    return std::string("t a r h e e l s");
}